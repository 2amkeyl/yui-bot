...
def make_embed(description: str, *, title: str = None, color: int = EMBED_COLOR,
...
def get_repeat_note(caller_id: int, target_id: int, cmd_name: str) -> str:
    ...
    if is_repeat:
        return "\n\n-# Kết quả này đã cố định cho hôm nay rồi, 6:00 sáng mai mới đổi được nha!"
    return ""
...
# ⚠️ 2026-09: api.waifu.pics đã die hẳn (NXDOMAIN từ nameserver gốc, không
# phải lỗi mạng/DNS phía mình) -> chuyển toàn bộ category sang nekos.best làm
# nguồn CHÍNH. Vẫn giữ waifu.pics làm fallback dự phòng, phòng khi bên đó fix
# lại domain trong tương lai thì code tự động khôi phục dùng song song.
WAIFU_CATEGORIES = {
...
# ── CACHE GIF theo (source, category) ──
GIF_CACHE_MAX = 30
...
ACTIONS: dict[str, dict] = {
    "hug": {"source": "nekos", "category": "hug", "lines": ["{a} lao vào ôm chầm lấy {b}", "{a} đè {b} ra ôm cứng ngắc, đéo cho thoát!"]},
    "cuddle": {"source": "nekos", "category": "cuddle", "lines": ["{a} rúc vào người {b} cọ cọ như con cờ hó!", "{a} dụi dụi vào người {b} tởm vãi lều!"]},
    "kiss": {"source": "nekos", "category": "kiss", "lines": ["{a} đè {b} ra bú mỏ chùn chụt!", "{a} cưỡng hôn {b} ướt nhẹp cmn hết cái mặt!"]},
    "pat": {"source": "nekos", "category": "pat", "lines": ["{a} xoa đầu {b} như xoa đầu cún!", "{a} vuốt ve cái đầu ngu ngốc của {b}!"]},
    "slap": {"source": "nekos", "category": "slap", "lines": ["{a} vả vỡ mẹ mõm {b}!", "{a} tát lật cmn mặt {b}, chừa cái thói láo cá chó đi!"]},
    "kill": {"source": "nekos", "category": "kick", "lines": ["{a} xiên chết cụ {b}!", "{a} tiễn {b} về chầu ông bà cmnl!"]},
    "poke": {"source": "nekos", "category": "poke", "lines": ["{a} chọc lủng cmn má {b}!", "{a} lấy ngón tay chọt chọt trêu chó {b}!"]},
    "highfive": {"source": "nekos", "category": "highfive", "lines": ["{a} đập tay {b} cái chát đau điếng cmn tay!", "{a} đập tay với {b} bung cmn móng!"]},
    "handhold": {"source": "nekos", "category": "handhold", "lines": ["{a} nắm chặt tay {b} đéo buông, ớn lạnh vãi!", "{a} đan tay {b} sến súa vãi cớt!"]},
    ...
    "cry": {"source": "nekos", "category": "cry", "lines": ["{a} khóc rống lên bám áo {b} ăn vạ!", "{a} rớt nước mắt cá sấu ướt cmn áo {b}!"]},
    "dance": {"source": "nekos", "category": "dance", "lines": ["{a} kéo {b} lên nhảy múa quạt như thằng ngáo đá!", "{a} và {b} quẩy tung cmn nóc, sập mẹ sàn rồi!"]},
    "nom": {"source": "nekos", "category": "nom", "lines": ["{a} ngoạm cmn một miếng {b}, ngon vãi cả cứt!", "{a} cắn nghiến {b} như cẩu đói ba ngày!"]},
    ...
    "snuggle": {"source": "nekos", "category": "cuddle", "lines": ["{a} rúc sát vào {b} như con đỉa bám!", "{a} chui tọt vào lòng {b}, đéo chịu ra!"]},
}
...
class FunCog(commands.Cog):
    ...
    def _soi_embed(self, title: str, user: discord.abc.User, percentage: int,
                    comment: str, note: str = "", guild: Optional[discord.Guild] = None) -> discord.Embed:
        desc = (
    ...
    async def soimayman(self, interaction: discord.Interaction, member: Optional[discord.Member] = None):
        ...
        tiers = [
            (0, "Tới thở cũng xui, nghiệp quật cmnr, hôm nay ra đường cẩn thận!"),
    ...
    async def hom_nay_an_gi(self, interaction: discord.Interaction):
        ...
        embed = make_embed(desc, title=f"{HEART} Hôm nay nạp gì? ({buoi})", guild=interaction.guild)
        await interaction.response.send_message(embed=embed)
