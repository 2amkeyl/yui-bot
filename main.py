...
from role_setup_cog import RoleSetupCog
...
class MinesView(discord.ui.View):
    ...
    def build_embed(self, *, status: str = "playing", win_amount: int = 0) -> discord.Embed:
        if status == "playing":
            desc, color = f"💣 {self.player.mention} đang dò mìn...", EMBED_COLOR
        elif status == "boom":
            ...
            desc, color = f"⌛ Ván của {self.player.mention} đã hết giờ, Yui tự rút tiền giúp rồi nha!", EMBED_COLOR
...
class MusicCog(commands.Cog):
    ...
    async def _play_next(self, guild: discord.Guild):
        ...
        def _after(error: Optional[Exception]):
            ...
            def _log_play_next_error(f):
                exc = f.exception()
                if exc:
                    log(f"Lỗi khi chuyển bài kế tiếp: {exc}", "warn")
            fut.add_done_callback(_log_play_next_error)
    ...
    @app_commands.command(name="play", description="Phát nhạc từ YouTube hoặc SoundCloud")
    @app_commands.describe(
        query="Link YouTube/SoundCloud hoặc tên bài hát để Yui tìm",
        source="Chọn nguồn khi tìm bằng tên bài (mặc định YouTube)"
    )
    @app_commands.choices(source=[
        Choice(name="YouTube", value="ytsearch1"),
        Choice(name="SoundCloud", value="scsearch1"),
    ])
    @app_commands.checks.cooldown(1, 3.0, key=lambda i: i.guild_id)
    async def play(self, interaction: discord.Interaction, query: str, source: Optional[Choice[str]] = None):
        ...
        search_prefix = source.value if source else "ytsearch1"
        search_target = query if is_url else f"{search_prefix}:{query}"
...
async def run_quests_with_progress(user_token: str, user: discord.User, mode: str, channel: discord.TextChannel) -> dict:
    def setup_and_get():
    ...
    for i, q in enumerate(data["actionable"]):
        if mode == "slow" and i > 0:
            ...
            try:
                delay_msg = await channel.send(content=user.mention, embed=make_embed(f"Yui mỏi tay quá, nghỉ uống trà chiều **{delay // 60} phút** rồi mới làm quest tiếp theo nha! :3", title=f"{HEART} Giờ nghỉ giải lao...", gif=True))
        ...
        prog_msg = None
        try: prog_msg = await channel.send(content=user.mention, embed=embed_prog)
        ...
        if is_success:
            ...
            try: 
                # Gửi thông báo xong lẻ tẻ và lưu lại
                succ_msg = await channel.send(content=user.mention, embed=make_embed(f"**{q_name}**\nLoại: {q_type}\n\n{HEART} Nhiệm vụ đã được Yui hoàn thành!", title=f"{HEART} Quest hoàn thành!", gif=True))
                success_msgs.append(succ_msg)
            except Exception as e: 
                log(f"Lỗi báo cáo quest xong: {e}", "warn")
        else:
            log(f"Quest {q_name} chạy thất bại do API không ghi nhận.", "error")

    return {"status": "success", "total": data["total"], "completed_before": data["completed_before"], "to_do": len(data["actionable"]), "success": success_count, "failed": len(data["actionable"]) - success_count, "success_msgs": success_msgs}

class ToSView(discord.ui.View):
    def __init__(self, user_token: str, mode: str):
        super().__init__(timeout=120)
    ...
    async def run_background_quests(self, user: discord.User, interaction: Optional[discord.Interaction] = None):
        target_channel = bot.get_channel(QUEST_CHANNEL_ID)
        result = await run_quests_with_progress(self.user_token, user, self.mode, target_channel)
        
        if result["status"] == "error":
            report_embed = make_embed(f"**Có lỗi kẹt lại nè:**\n{result['msg']}", title=f"{HEART} Ây da, có lỗi rồi", color=ERROR_COLOR)
        else:
            for msg in result.get("success_msgs", []):
                try:
                    await msg.delete()
            ...
            report_embed = make_embed(f"Tadaaa! Yui đã dọn dẹp sạch sẽ đống quest cho cậu rồi nè {HEART}", title=f"{HEART} BÁO CÁO HOÀN THÀNH")
        ...
        if interaction:
            try: await interaction.edit_original_response(embed=make_embed(f"Xong hết rồi đó, cậu xem kết quả ở kênh <#{QUEST_CHANNEL_ID}> nha! {HEART}", title=f"{HEART} Yui làm xong rồi!", gif=True))
            except Exception: pass
        
        try: await target_channel.send(content=user.mention, embed=report_embed)
        except Exception as e: log(f"Lỗi gửi báo cáo ra kênh cho {user.name}: {e}", "error")
...
class MyBot(commands.Bot):
    def __init__(self):
        ...
        # Bắt buộc cho tag_role_cog (on_member_update/on_member_join của member
        # khác + đọc guild.members). Nhớ bật thêm "Server Members Intent" ở
        # Discord Developer Portal -> app -> Bot -> Privileged Gateway Intents,
        # nếu không bật cả 2 chỗ thì bot sẽ không nhận được event, không báo lỗi.
    ...
    async def setup_hook(self):
        ...
        await self.add_cog(RoleSetupCog(self))
    ...
    async with bot.db_lock:
        ...
        if send_amount is None or send_amount <= 0:
            ...
            if given_today + send_amount > GIVE_DAILY_LIMIT:
                ...
                bot.yui_db[uid]["coins"] = max(0, bot.yui_db[uid]["coins"] - send_amount)
                bot.yui_db[uid]["give_today"] = given_today + send_amount
                bot.yui_db[rid]["coins"] += send_amount

    if error_msg:
        return await interaction.response.send_message(embed=make_embed(error_msg, color=ERROR_COLOR), ephemeral=True)
...
async def mine_game(interaction: discord.Interaction, bet: str, mines: app_commands.Range[int, MINES_MIN_MINES, MINES_MAX_MINES]):
    ...
    async with bot.db_lock: bot.yui_db[uid]["coins"] = max(0, bot.yui_db[uid]["coins"] - bet_amount)
    asyncio.create_task(bot.save_db())
    ...
    try:
        ...
        view.disable_all()
        async with bot.db_lock:
            bot.yui_db[uid]["coins"] = max(0, bot.yui_db[uid]["coins"] + bet_amount)
        asyncio.create_task(bot.save_db())
        log(f"Lỗi khi mở ván mine: {e}", "warn")
        try:
            await interaction.edit_original_response(embed=make_embed(
                "Có lỗi khi mở ván Dò Mìn, Yui đã hoàn tiền cho cậu rồi nha!", color=ERROR_COLOR
            ), view=None)
        except discord.HTTPException:
            pass
...
HELP_CATEGORIES = {
    "essential": ("Bot Nhạc", "Phát nhạc, hàng chờ, tua bài..", ["play", "skip", "pause", "resume", "queue", "nowplaying", "loop", "shuffle", "remove", "stop"]),
    "economy":   ("Kinh Tế", "Ví Yui Coin, điểm danh, bxh..", ["daily", "cash", "top", "give"]),
    "profile":   ("Hồ Sơ", "Xem avatar, banner cá nhân & server", ["avatar", "banner"]),
    "game":      ("Trò Chơi", "Nối từ, cá cược cùng Yui..", ["mine", "coinflip", "noitu", "noitu_stop"]),
    "unique":     ("Tính năng độc nhất", "Cày quest, huy hiệu..", ["badge", "hypesquad", "quest"]),
}
...
@bot.tree.command(name="quest", description="Gọi Yui ra cày Auto Quest nè :3")
...
async def quest_command(interaction: discord.Interaction, token: str, mode: Choice[str]):
    if interaction.channel_id != QUEST_CHANNEL_ID: 
        embed = make_embed(f"Này, qua đúng kênh <#{QUEST_CHANNEL_ID}> mà dùng lệnh nha! Yui hổng phục vụ ở đây đâu!", title=f"{HEART} Sai chỗ rồi kìa!", color=ERROR_COLOR, gif=True)
    ...
    await interaction.response.send_message(embed=make_embed(desc, title=f"{HEART} Lời Dặn Dò Của Yui Hirasawa {HEART}", gif=True), view=ToSView(user_token=token, mode=mode.value), ephemeral=True)

@bot.tree.command(name="hypesquad", description="Gán, đổi hoặc xoá huy hiệu HypeSquad cho tài khoản :3")
@app_commands.describe(token="Đưa Token", house="Chọn nhà HypeSquad bạn muốn nhận")
